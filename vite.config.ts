...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      '/api': {
        // httpsS адрес бэкенда (Docker или локальный)
        // Можно переопределить через переменную окружения VITE_PROXY_TARGET
        target: process.env.VITE_PROXY_TARGET || 'https://comment-api.up.railway.app',
        changeOrigin: true,
        secure: true // Отключаем проверку SSL для самоподписанных сертификатов (httpsS)
...
// proxyRes.headers['set-cookie'] = modifiedCookies;
      ...
      // Прокси для comments API (httpsS)
      // Используем bypass для проверки, является ли запрос API запросом
      '/comments': {
        target: process.env.VITE_PROXY_TARGET || 'https://comment-api.up.railway.app',
        changeOrigin: true,
